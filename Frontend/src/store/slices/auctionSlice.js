...
const auctionSlice = createSlice({
  ...
  reducers: {
    ...
    getAuctionDetailSuccess(state, action) {
      state.loading = false;
      state.auctionDetail = action.payload.auctionItems;
...
export const getAllAuctionItems = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/auctionitem/allitems`,
...
export const getMyAuctionItems = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/auctionitem/myitems`,
      { withCredentials: true }
    );
...
export const getAuctionDetail = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/auctionitem/auction/${id}`,
      { withCredentials: true }
    );
...
export const createAuction = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/auctionitem/create`,
      data,
      {
        withCredentials: true,
...
export const republishAuction = (id, data) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/auctionitem/item/republish/${id}`,
      data,
      {
        withCredentials: true,
...
export const deleteAuction = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_BASE_URL}/api/v1/auctionitem/delete/${id}`,
      {
        withCredentials: true,
      }
    );
